...
class DataModuleFromConfig(pl.LightningDataModule):# batchloader outputshape should be (b,h,w,c) and it will be permuted to (b,c,h,w) in autoencoder.get_input()
    ...
    def _train_dataloader(self):
        ...
        if isinstance(self.datasets["train"],ldm.data.joinaudiodataset_anylen.JoinManifestSpecs):
            ...
            return DataLoader(dataset, batch_size=self.batch_size,shuffle=True, sampler=None,
                          num_workers=self.num_workers, collate_fn=dataset.collate_fn,
    ...
    def _val_dataloader(self, shuffle=False):
        ...
        if isinstance(self.datasets["validation"],ldm.data.joinaudiodataset_anylen.JoinManifestSpecs):
            ...
            return DataLoader(dataset, batch_size=self.batch_size,shuffle=shuffle, sampler=None,
                          num_workers=self.num_workers, collate_fn=dataset.collate_fn,
                          worker_init_fn=init_fn)
...
class AudioCodecDataModuleFromConfig(DataModuleFromConfig):
    '''avoiding duplication of hyper-parameters in the config by gross patching here '''
    def __init__(self, batch_size, num_workers, text_column='captions', audio_column='location', train_path=None, validation_path=None,
